...
describe('Engine', () => {
  ...
  describe('addRule()', () => {
    describe('rule instance', () => {
      it('adds the rule', () => {
        let rule = new AR.Rule(factories.rule())
        expect(engine.rules.length).to.equal(0)
        engine.addRule(rule)
  ...
  describe('addOperator()', () => {
    it('adds the operator', () => {
      ...
      expect(engine.operators.get('startsWithLetter')).to.be.an.instanceof(AR.Operator)
    })

    it('accepts an operator instance', () => {
      expect(engine.operators.size).to.equal(10)
      let op = new AR.Operator('my-operator', _ => true)
  ...
  describe('addFact()', () => {
    ...
    it('allows a fact instance', () => {
      let options = { cache: false }
      let fact = new AR.Fact(FACT_NAME, 50, options)
      engine.addFact(fact)
      assertFact(engine)
  ...
  describe('removeFact()', () => {
    it('removes a Fact', () => {
      ...
      let fact = new AR.Fact('newFact', 50, { cache: false })
      engine.addFact(fact)
      expect(engine.facts.size).to.equal(1)
